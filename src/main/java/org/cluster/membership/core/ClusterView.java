...
public class ClusterView implements Serializable {
	...
	public ClusterView(DList nodes, ValuePrioritySet<ValuePriorityEntry<Node, Long>> suspectingNodesTimeout,
			ValuePrioritySet<ValuePriorityEntry<Node, Long>> failed, ValuePrioritySet<Message> rumorsToSend, ValuePrioritySet<Message> receivedRumors) {
		super();
		this.nodes = nodes;
	...
	public void updateMyView(List<Message> missingMessages) {
		...
		for(Message m : missingMessages) {
			if(m.getType().equals(MessageType.ADD_TO_CLUSTER)) nodes.add(m.getNode());
				...
				Long expirationTimeThisServer = DateTime.localTime(expirationTime, m.getGeneratedTimeZone(), Config.thisPeer().getTimeZone());
	...
	public ClusterView yourView(Node otherPeer) {
		...
		otherNodes.add(Config.thisPeer());
		...
		while(iterator.hasNext()) {
			...
			long convertedTime = DateTime.localTime(next.getValue(), Config.thisPeer().getTimeZone(), otherPeer.getTimeZone());
